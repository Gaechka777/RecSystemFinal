...
def BRec(LightningModule):
    def __init__(self, model_config, metrics, criterion, what2log=[]):
        super(BRec).__init__()
        self.n_items = model_config['n_items']
        self.d_model = model_config['d_model']
        self.heads = model_config.get('heads', 5)
        self.dropout = model_config.get('dropout', .5)
        self.n_layers = model_config.get('n_layers', 6)
        self.hidden_size = model_config.get('hidden_size', 2048)
        self.weights_path = model_config.get('weights_path', None)
        self.device = model_config.get('device', 'cpu')
        self.model = get_model(n_items=self.n_items, 
    ...
    def step(self, batch: Any, stage: str):
        ...
        if stage == 'train':
            loss_n_metrics = BRec._compute_loss_and_metrics(self, batch, outputs, self.loss, self.metrics)
        if stage == 'val':
            loss_n_metrics = BRec._compute_loss_and_metrics(self, batch, outputs, self.loss, self.metrics)
        if stage == 'test':
            loss_n_metrics = BRec._compute_loss_and_metrics(self, batch, outputs, self.loss, self.metrics)

        return loss_n_metrics, outputs
    
    def training_step(self, train_batch, batch_idx):
        loss_n_metrics, outputs = self.step(train_batch, 'train')
        loss = loss_n_metrics['loss']
        log = {'loss': loss.item()}
        log_metrics = {metr: loss_n_metrics[metr] for metr in self.logging_list}
        log.update(log_metrics)
        self.log_dict(log)
        return loss
    
    def validation_step(self, val_batch, batch_idx):
        loss_n_metrics, outputs = self.step(val_batch, 'val')
        loss = loss_n_metrics['loss']
        log = {'loss': loss.item()}
        log_metrics = {metr: loss_n_metrics[metr] for metr in self.logging_list}
        log.update(log_metrics)
    ...
    def _compute_loss_and_metrics(
        ...
        criterion: callable, 
        metrics: Dict[callable]):
        _, Y = batch
        loss = criterion(Y, outputs)
        result = {'loss': loss}
        for metric_name in metrics.keys():
            metric = metrics[metric_name]
            metric_value = metric(Y, outputs)
